...
class Dataset(DGLDataset):
        ...
        if phase == "test":
            if args.permutation_test:
                if args.permutation_feat != "None":
                    test_df = self.node_df[self.node_df['ReportYear'] >= 2019]
                    bg_df = self.node_df[self.node_df['ReportYear'] < 2019]
                    if args.permutation_feat in cat_feat:
                        ...
                        shuffle_cols = pd.DataFrame(test_df, columns=cols).values
                        np.random.shuffle(shuffle_cols)
                        for i in range(len(cols)):
                            test_df[feat+'_'+str(i)] = shuffle_cols[:,i]
                    else:
                        feat = args.permutation_feat
                        col = [item for item in self.node_df.columns[-33:] if feat in item]
                        shuffle_col = test_df[col].values
    ...
    def __getitem__(self, index):
        ...
        if len(src_ids) == 0 and len(dst_ids) == 0:
            t = one_node_df['ReportYear'].iloc[0]
            if t <= self.thre:  
                g = dgl.DGLGraph()
                g.add_nodes(1)
        else:
        ...
        g.ndata["time"] = torch.tensor(node_time, dtype=torch.float32)
...
def gen_label_weight(args):
    ...
    valid_df = valid_df[valid_df['ReportYear'] > 2014]
    node_df = pd.concat([node_df,valid_df])
    '''
