...
from dl_time import args, logger
...
def process_pred(pred_dict, scope):
    y_true, y_predscore, y_time = pred_dict["y_true"], pred_dict["y_predscore"], pred_dict["y_time"]
...
def cal_pred_metrics(pred_dict, scope):
    proc_pred_dict = process_pred(pred_dict, scope)
...
class PlotAly(object):
    def __init__(self, pred_dict, plot_path, scope):
        proc_pred_dict = process_pred(pred_dict, scope)
...
def plot_roc(fpr, tpr, roc_auc, save_path):
    ...
    roc['fpr'] = fpr
    roc['tpr'] = tpr
    if not Path(f"../test-time/roc").exists():
        os.mkdir(f"../test-time/roc")
    roc.to_csv(f"../test-time/roc/roc_{args.model}.csv",index=False)
...
def plot_confusion_mat(y_true, y_pred, labels, save_path):
    ...
    cm['true'] = y_true
    cm['predict'] = y_pred
    if not Path(f"../test-time/cm").exists():
        os.mkdir(f"../test-time/cm")
    cm.to_csv(f"../test-time/cm/cm_{args.model}.csv",index=False)
